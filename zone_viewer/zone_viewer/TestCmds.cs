...
namespace zone_viewer
{
    ...
    public class HelloWorld : IExternalCommand
    {
        public Result Execute(ExternalCommandData cmdData, ref string msg, ElementSet elems)
        {
            ...
            helloDlg.MainContent = "This API lists all levels of current project, and allows user to define construction zones on level views";
            helloDlg.Show();
    ...
    public class CmdSnoopDb : IExternalCommand
    {
        public Result Execute(ExternalCommandData cmdData, ref string msg, ElementSet elems)
        {
            ...
            try
            {
                ...
                ICollection<ElementId> founds = new FilteredElementCollector(doc).OfClass(typeof(Level)).OfCategory(BuiltInCategory.OST_Levels).ToElementIds();
                //FilteredElementCollector elemTypeCtor = (new FilteredElementCollector(doc)).WhereElementIsElementType();
                //ICollection<Element> founds;
                //foreach (ElementId level_id in levels){
                //    Element level doc.GetElement(level_id));
                //}


                ArrayList objs = new ArrayList();
                foreach (ElementId e_id in founds)
                {
                    objs.Add(doc.GetElement(e_id));
                }
    ...
    public class CmdSnoopActiveView : IExternalCommand
    {
        public Result Execute(ExternalCommandData cmdData, ref string msg, ElementSet elems)
        {
            ...
            try
            {
                ...
                {
                    TaskDialog.Show("zone_viewer", "The document must have an active view!");
                    return Result.Cancelled;
                }
    ...
    public class CmdSampleMenuItemCallback : IExternalCommand
    {
        public Result Execute(ExternalCommandData cmdData, ref string msg, ElementSet elems)
        {
            ...
            try
            {
                MessageBox.Show("Called back into zone_viewer by picking toolbar or menu item");
                result = Result.Succeeded;
            }
            catch (System.Exception e)
            {
